        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16" style={{ flex: 1, minHeight: 0, alignItems: 'center' }}>
          ...
          <motion.div initial={{ opacity: 0, x: -50 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-cinzel font-bold" style={{ fontSize: 'clamp(28px,3.5vw,44px)', color: COLOR }}>₹1,500</div>
              <p style={{ fontSize: 'clamp(11px,1.1vw,13px)', color: 'rgba(245,236,215,0.65)', marginTop: '4px' }}>60 min · Online / In-person</p>
              <Link href="/booking"
                className="mt-5 inline-block font-cinzel font-semibold uppercase transition-all duration-300 hover:brightness-125"
                style={{ fontSize: 'clamp(11px,1.1vw,13px)', letterSpacing: '2px', color: '#fff', background: 'linear-gradient(135deg, #7B3FD4, #9B59B6)', padding: 'clamp(8px,1.2vw,12px) clamp(20px,2.5vw,32px)', borderRadius: '3px' }}>
